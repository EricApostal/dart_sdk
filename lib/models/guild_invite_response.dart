// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, unused_import, invalid_annotation_target, unnecessary_import

import 'package:dart_mappable/dart_mappable.dart';

import 'channel_partial_response.dart';
import 'guild_invite_response_guild.dart';
import 'guild_invite_response_type_type.dart';
import 'user_partial_response.dart';

part 'guild_invite_response.mapper.dart';

@MappableClass()
class GuildInviteResponse with GuildInviteResponseMappable {
  const GuildInviteResponse({
    required this.code,
    required this.type,
    required this.guild,
    required this.channel,
    required this.memberCount,
    required this.presenceCount,
    required this.temporary,
    this.inviter,
    this.expiresAt,
  });

  final String code;
  final GuildInviteResponseTypeType type;
  final GuildInviteResponseGuild guild;
  final ChannelPartialResponse channel;
  @MappableField(key: 'member_count')
  final int memberCount;
  @MappableField(key: 'presence_count')
  final int presenceCount;
  final bool temporary;
  final UserPartialResponse? inviter;
  @MappableField(key: 'expires_at')
  final DateTime? expiresAt;

  static GuildInviteResponse fromJson(Map<String, dynamic> json) =>
      GuildInviteResponseMapper.fromJson(json);
}
